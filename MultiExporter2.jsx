...
var nyt_png_exporter = {
    ...
    hide_all_layers: function() {
		...
		for (var i = 0; i < n; ++i) {
			...
			// If the layer name contains "IDEN", skip hiding it
			if (lyr_name.indexOf("IDEN") !== -1) {
				continue;
			}
			
			// Layers starting with + are always turned on
			if (lyr_name.match(/^\+/)) {
				layer.visible = true;
			} 
			// Layers starting with -, default layer name, or "nyt_exporter_info" are skipped
			else if (lyr_name.match(/nyt_exporter_info/) || lyr_name.match(/^Layer /) || lyr_name.match(/^\-/)) {
				layer.visible = false;
			} 
			// All other layers are hidden for export
			else {
				layer.visible = false;
				export_count++;
			}
